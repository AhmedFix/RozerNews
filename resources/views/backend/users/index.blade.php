    ...
    <div class="roles-permissions">
        <div class="flex items-center justify-between mb-6">
            ...
            <div class="flex flex-wrap items-center">
                <a href="{{ route('user.create') }}" class="bg-gray-200 text-gray-700 text-sm uppercase py-2 px-4 flex items-center rounded">
        ...
        <div class="mt-8 bg-white rounded border-b-4 border-gray-300">
            <div class="flex flex-wrap items-center uppercase text-sm font-semibold bg-gray-300 text-gray-600 rounded-tl rounded-tr">
                ...
                <div class="w-1/12 px-4 py-3">Profile</div>
                <div class="w-2/12 px-4 py-3">Name</div>
                <div class="w-2/12 px-4 py-3">Email</div>
                <div class="w-3/12 px-4 py-3">Type</div>
                <div class="w-3/12 px-4 py-3 text-right">Action</div>
            </div>
            @foreach ($users as $user)
                <div class="flex flex-wrap items-center text-gray-700 border-t-2 border-l-4 border-r-4 border-gray-300">
                    <div class="w-1/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $user->id }}</div>
                    <div class="w-1/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">
                        <img class="w-10 h-10 rounded-full mr-1" src="{{$user->profile_picture_path  }}" alt="Avatar">
                    </div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $user->name }}</div>
                    <div class="w-2/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $user->email }}</div>
                    <div class="w-3/12 px-4 py-3 text-sm font-semibold text-gray-600 tracking-tight">{{ $user->type }}</div>
                    <div class="w-3/12 flex items-center justify-end px-3">
                        <a href="{{ route('user.edit',$user->id) }}" class="ml-1">
                        ...
                        <a href="{{ route('user.destroy', $user->id) }}" data-url="{!! URL::route('user.destroy', $user->id) !!}" class="deleteuser ml-1 bg-gray-600 block p-1 border border-gray-600 rounded-sm">
